...
export class ProductService {
  ...
  public async getAllProducts(
    ...
    return await this.contentRepository
      ...
      .andWhere(query.sku ? 'UPPER(content.sku) = UPPER(:sku)' : '1=1', {
        sku: query.sku,
      })
      .andWhere(query.name ? 'UPPER(content.name) LIKE UPPER(:name)' : '1=1', {
        name: `%${query.name ?? ''}%`,
      })
      .andWhere(
        query.brand ? 'UPPER(content.brand) LIKE UPPER(:brand)' : '1=1',
        { brand: `%${query.brand ?? ''}%` },
      )
      .andWhere(
        query.model ? 'UPPER(content.model) LIKE UPPER(:model)' : '1=1',
        { model: `%${query.model ?? ''}%` },
      )
      .andWhere(
        query.category
          ? 'UPPER(content.category) LIKE UPPER(:category)'
          : '1=1',
        { category: `%${query.category ?? ''}%` },
      )
      .andWhere(
        query.color ? 'UPPER(content.color) LIKE UPPER(:color)' : '1=1',
        { color: `%${query.color ?? ''}%` },
      )
      .andWhere(
        query.currency
          ? 'UPPER(content.currency) LIKE UPPER(:currency)'
          : '1=1',
        { currency: `%${query.currency ?? ''}%` },
      )
      .andWhere(query.minPrice ? 'content.price >= :minPrice' : '1=1', {
        minPrice: query.minPrice,
      })
      .andWhere(query.maxPrice ? 'content.price <= :maxPrice' : '1=1', {
        maxPrice: query.maxPrice,
      })
      .andWhere(query.minStock ? 'content.stock >= :minStock' : '1=1', {
        minStock: query.minStock,
      })
      .andWhere(query.maxStock ? 'content.stock <= :maxStock' : '1=1', {
