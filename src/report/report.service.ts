...
export class ReportService {
  ...
  public async getInactiveProductsPercentage(): Promise<number> {
    ...
    const inactiveProducts = await this.contentRepository.countBy({
      active: false,
    });
    return Math.round((inactiveProducts / totalProducts) * 100);
  }

  public async getActiveProductsPercentage(
    minDate: Date,
    maxDate: Date,
    minPrice?: number,
    maxPrice?: number,
    ...
    const totalProducts = await this.contentRepository.count();
    if (totalProducts === 0) return 0;
    const activeProducts = await this.contentRepository
      .createQueryBuilder('content')
      .where('content.active = :active', { active: true })
      .andWhere('content.createdAt BETWEEN :minDate AND :maxDate', {
        minDate: minDate,
        maxDate: maxDate,
      })
      .andWhere(minPrice ? 'content.price >= :minPrice' : '1=1', {
        minPrice: minPrice,
      })
      .andWhere(maxPrice ? 'content.price <= :maxPrice' : '1=1', {
        maxPrice: maxPrice,
      })
      .getCount();
